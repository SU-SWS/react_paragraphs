import React from 'react';
import {Droppable, Draggable} from "react-beautiful-dnd";
import styled from 'styled-components'
import {DrupalContext} from "../WidgetManager";
import {FlexDiv} from "./Atoms/FlexDiv";

const ToolboxWrapper = styled.div`
  border: 1px solid #C0C0C0;
  background: #F0F0EE;
  padding: 20px;
  max-height: calc(100vh - 120px);
  position: sticky;
  top: 80px;
  margin: -31px -19px -19px 20px;
  width: 200px;
  max-width: 200px;
  overflow: auto;
`;

const Toolwrapper = styled.div`
  background: #fff;
  margin: 10px;
  padding: 10px;
  border: 1px solid #ccc;
  box-shadow: 2px 2px 3px rgba(0, 0, 0, 0.13);
  width: 100px;
  min-height: 75px;
  text-align: center;
  font-weight: 600;
  font-size: 15px;
  line-height:23px;
  color: #4d4f53;

  img {
    max-width: 80px;
    max-height: 40px;
    margin: 0 auto 3px;
    display: block;
  }
`;

export const Toolbox = ({tools}) => {

  const getAsterickInfo = () => {
    let astericks = [];
    const asterick = '*';

    tools.map(tool => {
      if (tool.minWidth > 1) {
        if (!astericks.includes(12 / tool.minWidth)) {
          astericks.push(12 / tool.minWidth);
        }

        const repeatAsterick = astericks.findIndex(item => item === 12 / tool.minWidth) + 1;
        tool.asterick = asterick.repeat(repeatAsterick);
      }
    })

    return (
      <div>
        {astericks.map((line, index) => (
          <em key={index}>
            {asterick.repeat(index + 1)}Limit {line} per row
            <br/>
          </em>
        ))}
      </div>
    )
  }

  return (
    <ToolboxWrapper>
      <h3 className="fieldset-legend">Drag and drop Paragraphs into Rows</h3>
      {getAsterickInfo()}
      <Droppable
        droppableId="toolbox"
        type="item"
        direction="horizontal"
        isDropDisabled={true}
      >
        {provided => (
          <div
            {...provided.droppableProps}
            ref={provided.innerRef}
          >
            <FlexDiv column={true}>
              {tools.map((tool, weight) => (
                <Tool
                  key={`tool-${tool.id}`}
                  weight={weight}
                  machine_name={tool.id}
                  label={tool.label}
                  icon={tool.icon}
                  minWidth={tool.minWidth}
                  description={tool.description}
                  asterick={tool.asterick}
                />
              ))}
              {provided.placeholder}
            </FlexDiv>
          </div>
        )}
      </Droppable>
    </ToolboxWrapper>
  )
};

const Tool = ({weight, machine_name, label, description, icon, minWidth, asterick}) => {

  if (!icon) {
    icon = 'data:image/png;base64,';
  }

  return (
    <DrupalContext.Consumer>
      {drupalContext =>

        <Draggable
          draggableId={machine_name}
          index={weight}
        >
          {provided => (
            <Toolwrapper
              ref={provided.innerRef}
              {...provided.draggableProps}
              {...provided.dragHandleProps}
              onDoubleClick={(e) => drupalContext.addToolToBottom(machine_name, e)}
              id={`tool-${machine_name}`}
            >
              <img src={icon} alt="" role="presentation" title={description}/>

              <div className="label">{label}{asterick}</div>
            </Toolwrapper>
          )}
        </Draggable>

      }
    </DrupalContext.Consumer>
  )

};
